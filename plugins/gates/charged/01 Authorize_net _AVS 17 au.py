...
<a href="https://t.me/ByDog3r">⊁</a> <code>{data_bin[4]} {data_bin[5]}</code>"""
        ...
        async with session.get(
            ...
            try:
                ...
                login = ScrapInfo().getStr(
                    await response.text(), 'x_login" value="', '" />'
                )
                hash_ = ScrapInfo().getStr(
                    await response.text(), 'x_fp_hash" value="', '" />'
                )
                invoice = ScrapInfo().getStr(
                    await response.text(), 'invoice_num" value="', '" />'
                )
                time_stamp = ScrapInfo().getStr(
                    await response.text(), 'x_fp_timestamp" value="', '" />'
                )
                fp = ScrapInfo().getStr(
                    await response.text(), 'x_fp_sequence" value="', '" />'
                )
