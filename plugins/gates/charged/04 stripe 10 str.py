...
import string, random, time, rsa, base64, names, json, re, asyncio, aiohttp
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(
            ...
            if "Your card has insufficient funds." in await response1.text():
                ...
                "Your card's security code or expiration date is incorrect. ✅"
                in await response1.text()
            ):
                msgx = "APPROVED CCN"
                resultado = "Your card's security code or expiration date is incorrect."

            elif "success" in await response1.text():
                msgx = "APPROVED ✅"
                resultado = "10.00$ Charged"

            elif "Your card's security code is invalid." in await response1.text():
            ...
            elif "Your card was declined." in await response1.text():
                msgx = "DECLINED ❌"
                ...
                msgx = "DECLINED ❌"
                try:
                    resultado = await response1.json()["error"]["message"]
                except:
