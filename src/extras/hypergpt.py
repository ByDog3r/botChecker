...
async def fb_llama(generator):
        llama = hyper.chatbot(generator, model="llama")
        result = str(llama.response).replace("##", "**")
        return result

async def gpt(generator):
        chatgpt = hyper.chatbot(generator, model="gpt")
        result = str(chatgpt.response).replace("##", "**")
        return result

async def image_gen(generator):
        imageai = hyper.generate_image(generator, model="3d")
        result = imageai.result
        return result
...
# Generate Image
#print("\nGenerate Image: ")
#imageai = hyper.generate_image("Hacker pedorro", model="3d")
